...
function ExperienceDialog({
  ...
  useEffect(() => {
    const nextForm: ExperienceFormData = experience
      ? {
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Button
                ...
                size="sm"
                variant="outline"
                onClick={() => setForm((f) => ({ ...f, position_fr: f.position_en }))}
              >
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Button
                ...
                size="sm"
                variant="outline"
                onClick={() => setForm((f) => ({ ...f, description_fr: f.description_en }))}
              >
                {locale === "fr" ? "Copier EN -> FR" : "Copy EN -> FR"}
              </Button>
            </div>
...
export default function AdminExperiencePage() {
  ...
  const fetchData = useCallback(async (activePage: number) => {
    ...
    try {
      ...
      const data = await fetchJson<PaginatedResponse<Experience>>(
        `/api/experience?${endpointParams.toString()}`
      );
  ...
  useEffect(() => {
    setSelectedIds((current) => {
      const availableIds = new Set(experiences.map((item) => item.id));
  ...
  const allSelected = useMemo(
    () => experiences.length > 0 && experiences.every((item) => selectedIds.has(item.id)),
  ...
  const translationCounts = useMemo(() => {
    ...
    for (const item of experiences) {
      const hasEn = Boolean(item.position_en?.trim());
      const hasFr = Boolean(item.position_fr?.trim());
  ...
  const toggleAll = (checked: boolean) => {
    setSelectedIds(() => {
      ...
      return new Set(experiences.map((item) => item.id));
    });
  };
  ...
  const duplicateExperience = async (id: string) => {
    try {
      await fetchMutation(`/api/experience/${id}/duplicate`, { method: "POST" });
      toast.success(t.common.savedSuccessfully);
      await refreshAfterMutation();
    } catch (error) {
      toast.error(error instanceof Error ? error.message : t.common.errorOccurred);
  ...
  const applyBulkAction = async () => {
    ...
    try {
      await fetchMutation("/api/experience/bulk", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    <div className="space-y-8">
      ...
      {experiences.length === 0 ? (
        ...
        <div className="space-y-4">
          ...
          />
          {experiences.map((exp) => {
            const position = localizedText(previewLang, exp.position_en, exp.position_fr);
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                          toggleSelection(exp.id, checked === true)
                        }
                      />
                      <Briefcase className="h-4 w-4 text-muted-foreground" />
                      {position}
                      {(previewLang === "fr"
                        ? !exp.position_fr?.trim()
                        : !exp.position_en?.trim()) && (
